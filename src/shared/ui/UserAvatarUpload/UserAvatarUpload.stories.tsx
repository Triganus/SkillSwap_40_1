...
import { UserAvatarUpload, type UserAvatarUploadHandle, type UserAvatarUploadProps } from './UserAvatarUpload';
    ...
    <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button size="small" onClick={() => alert(ref.current?.getFile() ? 'Есть файл' : 'Нет файла')}>Проверить файл</Button>
          <Button size="small" variant="secondary" onClick={() => ref.current?.clear()}>Очистить</Button>
        </div>
      </div>
    </div>
  );
